angular.module('fusioApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/account/nav.html',
    "<nav class=fusio-nav><div class=fusio-nav-group><h3>Account</h3><ul><li><a ng-href=account>Profile</a></li><li><a ng-href=account/security>Security</a></li><li><a ng-href=account/grant>Grants</a></li><li><a ng-href=account/app>Apps</a></li><li><a ng-href=account/subscription>Subscriptions</a></li><li><a ng-href=account/plan>Plans</a></li><li><a ng-href=account/contract>Contracts</a></li><li><a ng-href=account/invoice>Invoices</a></li></ul></div></nav>"
  );
  ...
  $templateCache.put('app/login/login.html',
    "<div class=fusio-login-container><div uib-alert ng-class=\"'alert-danger'\" close=closeResponse() ng-if=\"response.success === false\">{{response.message}}</div><form id=loginForm method=POST ng-submit=login(user)><h1>Login</h1><div class=\"form-group has-feedback\"><input ng-model=user.username id=username required placeholder=Username class=form-control> <span class=\"ion-person form-control-feedback\"></span></div><div class=\"form-group has-feedback\"><input type=password ng-model=user.password id=password required placeholder=Password class=form-control> <span class=\"ion-key form-control-feedback\"></span></div><div class=\"form-group has-feedback\"><button type=submit ng-disabled=\"user.username == '' || user.password == ''\" class=\"btn btn-block btn-default\">Login</button></div></form><div class=fusio-login-ruler></div><p class=\"text-center text-muted\"><small><a href=./register>Sign up</a> or <a href=./password/reset>Forgot password</a></small></p><button ng-if=\"isConfigured('facebook')\" ng-click=\"authenticate('facebook')\" class=\"btn btn-block btn-facebook\"><i class=ion-social-facebook></i> Sign in with Facebook</button> <button ng-if=\"isConfigured('google')\" ng-click=\"authenticate('google')\" class=\"btn btn-block btn-google-plus\"><i class=ion-social-google></i> Sign in with Google</button> <button ng-if=\"isConfigured('github')\" ng-click=\"authenticate('github')\" class=\"btn btn-block btn-github\"><i class=ion-social-github></i> Sign in with GitHub</button></div>"
  );
  ...
  $templateCache.put('app/account/contract/detail.html',
    "<div class=modal-header><h3 class=modal-title>Invoices</h3></div><div class=modal-body><table class=table><colgroup><col width=100><col width=*><col width=100><col width=100><col width=50></colgroup><thead><tr><th>Status</th><th>Invoice-Id</th><th>Amount</th><th>Date</th><th></th></tr></thead><tbody><tr ng-repeat=\"invoice in invoices\"><td><span ng-switch=invoice.status><span ng-switch-when=0><span class=\"label label-info fusio-badge\">Open</span></span> <span ng-switch-when=1><span class=\"label label-success fusio-badge\">Payed</span></span> <span ng-switch-when=2><span class=\"label label-danger fusio-badge\">Deleted</span></span></span></td><td>{{invoice.displayId}}</td><td>{{invoice.amount | currency : symbol : 2}}</td><td>{{invoice.insertDate | date: 'shortDate'}}</td><td><button class=\"btn btn-default\" ng-disabled=\"invoice.status != 0 || loading\" ng-click=\"payInvoice(provider, invoice)\">Pay</button></td></tr></tbody></table></div><div class=modal-footer><button type=button ng-click=close() class=\"btn btn-default\">Cancel</button></div>"
  );
